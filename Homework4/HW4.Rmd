...
```
```{r}
probabilities <- predict(model2, no_na_df, type = "response")
predicted.classes <- ifelse(probabilities > 0.5, 1, 0)
no_na_df$pred.class <- predicted.classes
table("Predictions" = no_na_df$pred.class, "Actual" = no_na_df$TARGET_FLAG)
```
```{r}
confusionMatrix(as.factor(predicted.classes), as.factor(no_na_df$TARGET_FLAG))

```


```{r}
curve <- roc(response = no_na_df$TARGET_FLAG, 
...
#### Model 4: GLMULTI
```{r}
 model4 <- glmulti(TARGET_FLAG ~ ., data = df_rm, level = 1, method="h", crit = "aic", plotty = FALSE, fitfunction = "glm", family=binomial, maxK = 5)
